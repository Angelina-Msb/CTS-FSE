...
-- 2. Top Rated Events
SELECT e.event_id,e.title,AVG(f.rating) avg_rating,COUNT(*) feedback_count
FROM Events e
...
SELECT e.city,COUNT(DISTINCT r.user_id) registrations
...
SELECT u.full_name,e.status,COUNT(*) event_count
FROM Events e
...
HAVING COUNT(*)=(
 SELECT MAX(cnt)
 FROM (SELECT COUNT(*) cnt FROM Sessions GROUP BY event_id)x
);
...
SELECT speaker_name,COUNT(*) sessions
FROM Sessions
...
SELECT user_id,event_id,COUNT(*) duplicates
FROM Registrations
